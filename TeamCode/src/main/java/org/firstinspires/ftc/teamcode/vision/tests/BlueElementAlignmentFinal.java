...
public class BlueElementAlignmentFinal extends OpenCvPipeline {
    ...
    private final int verticalAxisX = 216; // X-coordinate of the center vertical axis

    public BlueElementAlignmentFinal() {
        // Set the color range for detecting the blue element
        low = new Scalar(0, 0, 148.8, 0);
        high = new Scalar(167.2, 128.9, 255, 255); // Adjusted upper bound for blue
    }
    public BlueElementAlignmentFinal(Scalar lowerBound, Scalar upperBound) {
        low = lowerBound;
        high = upperBound;
    }
    ...
    public Mat processFrame(Mat input) {
        ...
        Imgproc.putText(input, "Blue", new Point(130, 10), Imgproc.FONT_HERSHEY_SIMPLEX, 0.4, new Scalar(0, 0, 255), 1);
    ...
    public boolean isAligned() {
        return Math.abs(rotationAngle) <= tolerance || Math.abs(rotationAngle - 90) <= tolerance;
    }
