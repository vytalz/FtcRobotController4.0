...
public class BlueElementDetector extends OpenCvPipeline {
    ...
    public BlueElementDetector() {
        // Set the color range for red
        low = new Scalar(0, 0, 148.8, 0);
        high = new Scalar(167.2, 128.9, 255, 255); // Adjusted upper bound for red
    }

    public BlueElementDetector(Scalar lowerBound, Scalar upperBound) {
        low = lowerBound;
        high = upperBound;
    }
