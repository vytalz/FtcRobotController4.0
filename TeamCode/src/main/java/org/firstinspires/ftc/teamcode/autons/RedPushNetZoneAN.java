...
@Autonomous(name = "RedPushAN")
public class RedPushNetZoneAN extends CommandOpMode {
...
//                .build();
        Trajectory splineToSampleOne = drive.trajectoryBuilder(startPose, Math.toRadians(90))
                .splineToSplineHeading(new Pose2d(37.4, 25.9),Math.toRadians(135))
                .build();
...
//        TrajectoryFollowerCommand autonomous = new TrajectoryFollowerCommand(drive, center);
        ...
        schedule(autonomous.andThen(new WaitCommand(1000).andThen(autonomous2)));
